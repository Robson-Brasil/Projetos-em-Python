...
def falar(audio):
    ...
    time.sleep(1.0)  # Adiciona uma pausa de 1 segundo após cada fala
...
def microfone():
    ...
    model = Model(r"D:\Vosk\model-br")
    rec = KaldiRecognizer(model, 16000)
    
    commando = None  # Initialize commando with a default value
    ...
    while True:
        data = stream.read(4096)  # Increased buffer size to 4096 bytes
        if len(data) == 0:
            break
